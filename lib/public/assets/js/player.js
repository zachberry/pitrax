...
//{
			...
			if(!this.isHidden)
			{
				...
				if(this.songsLoaded)
				{
					...
					{
						/*
						var songView = new SongView({model: songs.getByIndex(i)});
...
//}
	...
	var LibraryView_Dg = Backbone.View.extend(_.extend({}, Backbone.Events, {
		TABLE_MAX_SONGS: 200,
	...
	var LibraryView = Backbone.View.extend(_.extend({}, Backbone.Events, {
		...
		render: function() {
			...
			if(this.unrendered)
			{
				...
				//@HACKS
				/*
				$('tr').live('click', {view:this}, function(event) {
					console.log(event);
					var songId = $(this).attr('data-song-id');
					console.log(songId);
					var view = event.data.view;
					console.log(view);
					var song = view.model.get('playlist').songs.get(songId);
					console.log(song);

					view.model.play(song);
				});*/
			}
		...
		_scrollHandler: function(event) {
			// NEW IDEA:
				// figure out which tables we should be seeing
				// based on percentages (scrollbar)
			//ALSO:
				// move playing and selected out of song model and into queue model!
				//undefined - null = NaN;
			...
			var h = st / this.tableHeight;
			////console.log(h, parseInt(h), parseInt((h - parseInt(h)) * 100));

			
			var tableIndex = parseInt(h, 10);
			var perc = parseInt((h - tableIndex) * 100, 10);
...
/*
			...
			if(this.activeTableIndex !== tableIndex)
			{


				this._deactivateTable(tableIndex - 2);
				this._activateTable(tableIndex - 1);
				this._activateTable(tableIndex);
				this._activateTable(tableIndex + 1);
				this._deactivateTable(tableIndex + 2);
